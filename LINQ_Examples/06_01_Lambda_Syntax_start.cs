...
namespace LINQ_Examples
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            IEnumerable<Customer> stateQuery = customers.Where(blah => blah.State == "OR");

            Console.WriteLine("Here are all the customers that live in Oregon:");
            ...
            int cutOffPoint = 5;
            List<int> numbers = new List<int>() { 3, 6, 7, 4, 1, 8, 5, 9, 2 };

            IEnumerable<int> numsBelowCuttOff = numbers.Where(x => x <= cutOffPoint);

            Console.WriteLine("Here are all the numbers less than or equal to the cutoff, \nwhich is wet to 5:");

            foreach(int num in numsBelowCuttOff)
            {
            ...
            Console.WriteLine("The fully written out method results in {0}", ComputeSquare(5));

            Console.WriteLine();

            Console.WriteLine("The lambda expression result is {0}", ComputeTheSquare(5));

            Console.ReadKey();

        }

        public static int ComputeSquare(int value)
        {
            return value * value;
        }

        //Instead of ComputeSquare method below you can use Lambda expression:
        public static int ComputeTheSquare(int value) => value * value;
    }
    ...
    public class SomeClass
    {
        ...
        //Property using lambda(Read only, can't have a setter)
        public int X => x;
